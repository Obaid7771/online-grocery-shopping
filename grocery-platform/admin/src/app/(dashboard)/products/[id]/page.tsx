    ...
    async function fetchData() {
      try {
        const [productRes, categoriesRes] = await Promise.all([
          ...
          api.get("/categories?includeInactive=false"),
        ]);
        ...
        setImageUrls(urls.length > 0 ? urls : product.imageUrls || []);
        ...
        reset({
          ...
          unit: product.unit,
          unitStep: Number(product.unitStep),
          stockQuantity: product.stockQuantity ?? product.inventory?.quantity ?? 0,
          minStockThreshold: product.minStockThreshold ?? product.inventory?.lowStockThreshold ?? 5,
  ...
  const onSubmit = async (data: FormData) => {
    try {
      ...
      await api.put(`/products/${productId}`, {
        ...
        price: data.price,
        discountPrice: data.comparePrice || null, // Backend uses discountPrice
        unit: data.unit,
